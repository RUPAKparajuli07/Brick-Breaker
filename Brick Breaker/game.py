...
game_rows = 8  # Increase number of rows
game_columns = 14  # Increase number of columns
...
'''
    ...
    def __init__(self, x, y):
        ...
        self.radius = int(Window_width * 0.015)  # Adjust ball size based on window width
        self.x = x - self.radius
        self.y = y - int(Window_height * 0.0833)  # Adjust ball position based on window height
        self.rect = Rect(self.x, self.y, self.radius * 2, self.radius * 2)
        self.x_speed = int(Window_width * 0.005)  # Adjust ball speed based on window width
        self.y_speed = -int(Window_height * 0.0083)  # Adjust ball speed based on window height
        self.max_speed = int(Window_width * 0.00625)  # Adjust ball max speed based on window width
        self.game_over = 0

    def motion(self):
        collision_threshold = int(Window_width * 0.00625)  # Adjust collision threshold based on window width
    ...
    def draw(self):
        pygame.draw.circle(window, (0, 255, 0), (self.rect.x +
                                                  self.radius, self.rect.y + self.radius), self.radius)  # Change ball color
...
'''
    ...
    def __init__(self):
        ...
        self.height = int(Window_height * 0.0667)  # Adjust brick height based on window height
    ...
    def make_brick(self):
        ...
        for row in range(game_rows):
            ...
            for column in range(game_columns):  # changed variable name to 'column'
    ...
    def draw_brick(self):
        for row in self.bricks:
            for brick in row:
                ...
                if brick[0] != (0, 0, 0, 0):  # Only draw non-destroyed bricks
...
'''
    ...
    def __init__(self):
        ...
        self.height = int(Window_height * 0.0333)  # Adjust base height based on window height
        self.width = int(Window_width / game_columns)
        self.x = int((Window_width / 2) - (self.width / 2))
        self.y = Window_height - (self.height * 2)
        self.speed = int(Window_width * 0.01)  # Adjust base speed based on window width
